...
function usage {
  ...
  echo "                           demo_dashboard in the virtual"
  echo "                           environment."
...
function backup_environment {
  if [ $backup_env -eq 1 ]; then
    ...
    if [ -d /tmp/.demo_dashboard_environment/$JOB_NAME ]; then
      mv /tmp/.demo_dashboard_environment/$JOB_NAME /tmp/.demo_dashboard_environment/$JOB_NAME.old
      rm -rf /tmp/.demo_dashboard_environment/$JOB_NAME
    fi
    mkdir -p /tmp/.demo_dashboard_environment/$JOB_NAME
    cp -r $venv /tmp/.demo_dashboard_environment/$JOB_NAME/
    cp .environment_version /tmp/.demo_dashboard_environment/$JOB_NAME/
    # Remove the backup now that we've completed successfully
    rm -rf /tmp/.demo_dashboard_environment/$JOB_NAME.old
...
function restore_environment {
  if [ $restore_env -eq 1 ]; then
    ...
    cp -r /tmp/.demo_dashboard_environment/$JOB_NAME/.venv ./ || true
    cp -r /tmp/.demo_dashboard_environment/$JOB_NAME/.environment_version ./ || true
...
function install_venv {
  ...
  # Use Crate.io instead of pypi's mirrors since crate is faster and
  # more reliable. The "restricted" endpoint does not spider external
  # pages. No packages currently in use depend on this functionality,
  # which is very often the cause of slow and unreliable
  # installations.
  export PIP_USE_MIRRORS=false
  export PIP_INDEX_URL="http://restricted.crate.io/"
...
function run_tests {
  ...
  if [ $selenium -eq 1 ]; then
      ${command_wrapper} coverage run --branch --omit="*horizon*" -p $root/manage.py test $tested_apps --settings=$settings --with-selenium --with-cherrypyliveserver $testargs
    else
      ${command_wrapper} coverage run --branch --omit="*horizon*" -p $root/manage.py test $tested_apps --settings=$settings $testargs
  fi
  # get results of the demo_dashboard tests
  DASHBOARD_RESULT=$?

  if [ $with_coverage -eq 1 ]; then
    echo "Generating coverage reports"
    ${command_wrapper} coverage combine
    ${command_wrapper} coverage xml -i --omit='/usr*,setup.py,*egg*,.venv/*,*horizon/*'
    ${command_wrapper} coverage html -i --omit='/usr*,setup.py,*egg*,.venv/*,*horizon/*' -d reports
  fi
